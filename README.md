...
- 主区：当前会话的 Q&A 时间线卡片。问题块（「问」标签 + 左侧高亮条 + 加粗）与
  ...
  先转义后渲染防 XSS）。已完成卡片右上角垃圾桶图标可删除该条记录（各端同步，API：
...
- 内容宽度：头部「宽度」下拉三档 —— **窄**（860px，默认）/ **宽**（1180px）/
  **铺满**（撑满主容器，两侧留 16px），作用于问答卡片与底部输入框，本地记忆
  （localStorage）。
- 图标：引入**轻量自托管 SVG 图标库**（`public/icons.svg` sprite，21 枚 Lucide 风格
...
- `tests/run_tests.js`：**99 项**断言 —— 协议客户端单测（含超时/取消/错误）+
